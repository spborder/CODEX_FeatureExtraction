    ...
    def get_intensity_features(self, region_coords:list, return_type: Union[List [str], str]):
        ...
        if len(annotations_json[0]['annotation']['elements'])>0:
            for nuc_idx,nuc in tqdm(enumerate(annotations_json[0]['annotation']['elements'])):
                ...
                mean_frames = np.nanmean(masked_frames,axis = (-1,1))
                std_frames = np.nanstd(masked_frames,axis = (-1,1))
                max_frames = np.nanmax(masked_frames,axis = (-1,1))
                min_frames = np.nanmin(masked_frames,axis = (-1,1))
                median_frames = np.nanmedian(masked_frames,axis = (-1,1))
